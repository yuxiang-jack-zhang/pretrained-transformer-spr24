...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if type(document) != str: print(f"DOCUMENT TYPE = {type(document)}")

        # 1. Randomly truncate the document to a length no less than 4 characters,
        #    and no more than int(self.block_size*7/8) characters.
        truncate_len = int(torch.randint(4, self.block_size*7//8 + 1, (1,))[0])
        document_truncated = document[:truncate_len]

        # 2. Now, break the (truncated) document into three substrings: [prefix] [masked_content] [suffix]
        masked_len = int(torch.randint(1, 2 * truncate_len // 4, (1,))[0])
        start_idx = int(torch.randint(0, truncate_len - truncate_len // 4 + 1, (1,))[0])
 
        prefix = document_truncated[:start_idx]
        masked_content = document_truncated[start_idx : start_idx + masked_len]
        suffix = document_truncated[start_idx + masked_len:]
