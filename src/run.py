...
if args.function == 'pretrain':
    ...
    # Pretrain the model on the pretrain corpus 
    # (pretrain corpus is already loaded as pretrain_dataset)
    ...
    trainer_pretrain = trainer.Trainer(model, pretrain_dataset, None, tconf)
    ...
    if args.reading_params_path is not None:
        # If pretrained params are specified, load these parameters into the model
        model.load_state_dict(torch.load(args.reading_params_path))
        # use hyperparamters for finetuning WITH a pretrained model
    ...
    trainer_finetune = trainer.Trainer(model, finetune_dataset, None, tconf)
