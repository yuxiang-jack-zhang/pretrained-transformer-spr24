...
def apply_rotary_emb(x, rope_cache):
    ...
    # Truncate rope_cache if necessary, this won't do anything if its not necessary
    rope_cache_truncated = rope_cache[:T]

    # Modify dimensions of rope_cache so we can do matrix mult
    rope_cache_truncated = rope_cache_truncated.unsqueeze(0).unsqueeze(0)
    rope_cache_truncated = rope_cache_truncated.expand(B, nh, -1, -1, -1)
    rope_cache_truncated = torch.view_as_complex(rope_cache_truncated)
    # print(f"!!!rope_cache_truncated.shape = {rope_cache_truncated.shape}")

    # Match x dimensions to rope_cache
    rotated_x = x.view(B, nh, T, hs//2, 2)
    rotated_x = torch.view_as_complex(rotated_x)
    # print(f"!!!rotated_x.shape = {rotated_x.shape}")

    # Apply rope_cache
    rotated_x = rope_cache_truncated * rotated_x
    
    # Reshape rotated_x back to (B, nh, T, hs)
    rotated_x = torch.view_as_real(rotated_x)
    rotated_x = rotated_x.view(B, nh, T, hs)
    # print(f"!!!finalOutput.shape = {rotated_x.shape}")
